...
import { sql, sum, eq, and, lte, gte, lt } from 'drizzle-orm' 
...
const app = new Hono()
  .get(
    ...
    const [currentPeriod = { income: 0, expenses: 0, remaining: 0 }] = await fetchFinancialData(
        auth.userId,
        startDate,
        endDate,
    )

    const [lastPeriod = { income: 0, expenses: 0, remaining: 0}] = await fetchFinancialData (
        auth.userId,
        lastPeriodStart,
        lastPeriodEnd,
    );
    ...
    .select({
     ...
     .from(transactions)
        ...
        .groupBy(categories.name) // Having same category name once
